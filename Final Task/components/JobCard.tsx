...
//     if (!token) {
//       toast.error('You need to be logged in to bookmark a job.');
//       // 3. Use router.push for client-side navigation. This does NOT throw an error.
//       setTimeout(() => {
//         router.push('/login');
//       }, 4000); // The delay is fine, but the navigation method must change.
//       return;
//     }
...
import Tag from './tag';
import { useSession } from 'next-auth/react';
// IMPORTANT: Use `useRouter` for client-side navigation, NOT `redirect`.
import { useRouter } from 'next/navigation';
import { useState, useEffect } from 'react';
import { CiBookmarkCheck, CiBookmark } from 'react-icons/ci';
import {
...
export default function JobCard({ job, index }: { job: Opportunity; index: number }) {
  ...
  // The `skip` option is crucial. It prevents the query from running if there's no token.
  const { data } = useGetBookmarksQuery(token, {
    skip: !token,
  });
  ...
  const handleBookmarkToggle = async (e: React.MouseEvent) => {
    ...
    if (!token) {
      toast.error('You need to be logged in to bookmark a job.');
      // Use router.push for client-side navigation. This will NOT crash Cypress.
      setTimeout(() => {
        router.push('/login');
      }, 4000);
      return;
    ...
    try {
      if (isBookmarked) {
        await removeBookmark({ jobId: job.id, token }).unwrap();
        toast.success('Removed from bookmarks');
      } else {
        await addBookmark({ jobId: job.id, token }).unwrap();
        toast.success('Added to bookmarks');
      }
      // Note: RTK Query's `invalidatesTags` will trigger a re-fetch,
      // automatically updating the UI. The manual `setIsBookmarked` below provides
      // a faster "optimistic" update for a better user experience.
      setIsBookmarked(!isBookmarked);
    } catch (error: any) {
      const errorMessage = error?.data?.message || 'Something went wrong while toggling bookmark.';
      toast.error(errorMessage);
      console.error('Bookmark toggle failed:', error);
    }
  };
